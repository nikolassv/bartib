...
impl FromStr for Activity {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		let starttime = match NaiveDateTime::parse_from_str(time_parts[0].trim(), conf::FORMAT_DATETIME) {
			Ok(t) => t,
			Err(_) => {
				return Err(ActivityError::DateTimeParseError)
			}
		};
...
mod tests {
	...
	#[test]
	fn display() {
		let mut t = Activity::start(project::Project("test project| 1".to_string()), "test\\description".to_string());		
	...
	fn from_str_stopped_activity() {
		let t = Activity::from_str("2021-02-16 16:14 - 2021-02-16 18:23 | test project | test description").unwrap();
	...
	fn from_str_escaped_chars() {
		let t = Activity::from_str("2021-02-16 16:14 - 2021-02-16 18:23 | test project\\| 1 | test\\\\description").unwrap();
	...
	#[test]
	fn string_roundtrip() {
		let mut t = Activity::start(project::Project("ex\\ample\\\\pro|ject".to_string()), "e\\\\xam|||ple tas\t\t\nk".to_string());
		t.stop();
